...
$query_qnt_scrap_analise = "SELECT COUNT(sc.id) AS qnt_scrap_analise FROM scrap sc WHERE sc.status = 1 ";

// Acessa o IF quando há parâmetros de pesquisa   
if(!empty($dados_requisicao['search']['value'])) {
    $query_qnt_scrap_analise .= " OR sc.serial LIKE :serial
                     ...
                     OR (SELECT nomr FROM usuario WHERE usuario.id = sc.usuario_id) LIKE :usuario";
}
...
$query_scrap_analise = "SELECT 
                        ...
                        FROM scrap sc WHERE sc.status = 1 ";

// Acessa o IF quando há parâmetros de pesquisa   
if(!empty($dados_requisicao['search']['value'])) {
    $query_scrap_analise .= " OR sc.serial LIKE :serial
                     OR ELT(sc.reincidente, 'SIM', 'NÃO') LIKE :reincidente
                     OR ELT(sc.area, 'SMD', 'FA') LIKE :area
                     OR (SELECT cimcode FROM produto WHERE produto.id = sc.produto_id) LIKE :produto
                     OR (SELECT SUM(sp.valor_t) FROM scrap_pn sp WHERE sp.scrap_id = sc.id) LIKE :total;
...
if(!empty($dados_requisicao['search']['value'])) {
    ...
    $result_scrap_analise->bindParam(':usario', $valor_pesq, PDO::PARAM_STR);
}
