                        ...
                        <div class="col-md-10">
                            <div class="form-group">
                                <input type="text" id="linhaInput" class="form-control"
                                    placeholder="Insira o nome do linha">
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
                    ...
                    <div class="card shadow">
                        <div class="card-header border-0 bg-gradient-visteon-barra">
                            <div class="row align-items-center ">
                                <div class="col">
                                    <h5 class="mb-0">Linhas Cadastrados</h5>
                                </div>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <!-- Projects table -->
                            <table id="listar-linha" class="display" style="width:100%">
                                <thead>
                                    <tr>
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="id_linha" id="id_linha" autofocus="" required="">
                        </div>
                    </div>
                ...
                <div class="card-footer py-4">
                    <div style="text-align: right">
                        ...
                        <button id="excluirLinha " type="submit" class="btn btn-danger excluirLinha">
...
<script>
    const selectElement = document.querySelector("#linhaInput");


function updateTables() {
// Supondo que você esteja utilizando DataTables
    $('#listar-linha').DataTable().ajax.reload(); // Recarrega os dados da tabela
}
...
document.getElementById('saveDataButton').addEventListener('click', function () {
    ...
    if (linha == "") {
        alertify.error('Insira o linha!');
    } else {
        fetch('consultas/linhas/insert.php', {
                method: 'POST',
                headers: {
...
$(document).ready(function () {
    ...
    var table = $('#listar-linha').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "consultas/linhas/table.php",
        language: {
            url: 'http://localhost:8080/scrap/assets/js/dataTables-pt-BR.json',
        },
    });
    $('#listar-linha tbody').on('click', 'tr', function () {
        ...
        $.ajax({
            url: 'consultas/linhas/consultaDetail.php',
            type: 'post',
            dataType: 'json',
            data: {
                id: data[1]
    ...
    $(this).on('click', '.editModal', function (e) {
        ...
        if ($("#descricao_d").val()) {
            alertify.confirm("Inserir", "Deseja inserir?",
                function () {
                    $.getJSON('consultas/linhas/edit.php?search=', {
                        id: $("#id_linha").val(),
    ...
    $(this).on('click', '.excluirLinha', function (e) {
        e.preventDefault();
        //alert($('#inputRate').val());
        var id = $("#id_linha").val();
        alertify.confirm("Excluir", "Deseja excluir Linha?",
            function () {
                $.getJSON('consultas/linhas/delete.php?search=', {
                    id: id,
                    ajax: 'true'
                }, function (j) {
